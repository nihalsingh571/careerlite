...
def search_log(request):
    ...
    all_logs = SearchResult.objects.all()
    today_count = all_logs.filter(search_on__date=today).count()
    
    high_result_count = sum(1 for log in all_logs if int(log.job_post) > 50)
    
    one_hour_ago = timezone.now() - timedelta(hours=1)
    recent_count = all_logs.filter(search_on__gte=one_hour_ago).count()
