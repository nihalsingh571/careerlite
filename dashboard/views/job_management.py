...
def status_change(request, post_id):
    ...
    # Temporarily disable Haystack signals to avoid Elasticsearch connection errors
    try:
        with transaction.atomic():
            # Temporarily disconnect Haystack signals
            post_save.disconnect(RealtimeSignalProcessor.handle_save)
            post_delete.disconnect(RealtimeSignalProcessor.handle_delete)
            ...
            # Reconnect signals
            post_save.connect(RealtimeSignalProcessor.handle_save)
            post_delete.connect(RealtimeSignalProcessor.handle_delete)
    except Exception as e:
        # If there's any error, still save the job post without search indexing
        if post.status == "Live":
            post.status = "Expired"
            post.save()
        else:
